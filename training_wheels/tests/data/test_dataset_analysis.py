from training_wheels.data.dataset_analysis import get_label_imbalance_array, get_label_imbalance_max, get_scale_array, get_deviation

import numpy as np
import statistics as st
import pytest

def compare_2d_array(expected, actual):
    for i in range(len(expected)):
        for j in range(len(expected[i])):
            assert actual[i, j] == expected[i, j]

def compare_3d_array(expected, actual):
    for i in range(len(expected)):
        for j in range(len(expected[i])):
...
def test_label_imbalance_1():
    ...
    compare_2d_array(expected, actual)
    ...
    assert get_label_imbalance_max([1,1,2,2,1,1,1,3,4,4]) == .4
    assert get_label_imbalance_max([1,1,2,2,1,1,1,3,4,4], raw_count=True) == 4

    with pytest.raises(RuntimeError):
        get_label_imbalance_array([1,1,1,1,1])
    
    with pytest.raises(RuntimeError):
        get_label_imbalance_max([1,1,1,1])
...
def test_scale():
    ...
    compare_3d_array(expected, actual)
